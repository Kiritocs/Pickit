...
namespace PickIt
{
    public class PickIt : BaseSettingsPlugin<PickItSettings>
    {
        ...
        // TODO: Move to file or search base item on path
        ...
        private void DrawIgnoredCellsSettings()
        {
            ...
            const ImGuiWindowFlags moveableFlag = ImGuiWindowFlags.NoResize | ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse |
                                                  ImGuiWindowFlags.NoCollapse | ImGuiWindowFlags.AlwaysAutoResize | ImGuiWindowFlags.NoBringToFrontOnFocus | ImGuiWindowFlags.NoFocusOnAppearing;
            ...
            if (!ImGui.Begin($"{Name}", ref opened, Settings.MoveInventoryView.Value ? moveableFlag : nonMoveableFlag))
            {
                return;
            }
            ...
            for (var i = 0; i < 5; i++)
            {
                for (var j = 0; j < 11; j++)
                {
                    ...
                    if (ImGui.Checkbox($"##{currentCellNumber++}IgnoredCells", ref toggled))
                    {
                        inventorySlots[i, j] ^= 1;
                    }

                    ImGui.SameLine();
                }
                var lastToggled = Convert.ToBoolean(inventorySlots[i, 11]);
                if (ImGui.Checkbox($"##{currentCellNumber++}IgnoredCells", ref lastToggled))
                {
                    inventorySlots[i, 11] ^= 1;
                }
            }
        ...
        private IEnumerator TryToPickV2(CustomItem pickItItem, LabelOnGround portalLabel)
        {
            ...
            while (tryCount < 3)
            {
                ...
                {
                    _fullWork = true;
                    //DebugWindow.LogDebug($"x,y outside game window. Label: {centerOfItemLabel} Window: {rectangleOfGameWindow}", 5, Color.Red);
                    yield break;
                }
                ...
                if (pickItItem.IsTargeted())
                {
                    ...
                    if (IsPortalNearby(portalLabel, pickItItem.LabelOnGround) && !IsPortalTargeted(portalLabel))
                    {
                        ...
                        if (IsPortalNearby(portalLabel, pickItItem.LabelOnGround) && !IsPortalTargeted(portalLabel))
                        {
                            ...
                            yield return new WaitTime(this._random.Next(40, 60));
                        }
                    }
        ...
        public HashSet<string> LoadPickit(string fileName)
        {
            ...
            DebugWindow.LogDebug($"PICKIT :: (Re)Loaded {fileName}");
            return hashSet;
        }
